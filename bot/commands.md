...
| `/alerts` | Choose one proactive PnL alert type (out-of-range, IL tolerance, or breakeven) and its threshold. See [Notifications](notifications.md). |
| `/apikey` | Get or rotate your long-lived signals API key, for polling the [Signals API](../signals/index.md) with your own agent. |
...
- **Mute (proactive alerts)** — IL-tolerance and breakeven alerts carry
  a **🔕 Mute** button that silences the active proactive alert type for
  that one position.
